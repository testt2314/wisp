...
class WhisperTranscriber:
    ...
    def _transcribe_with_faster_whisper(self, audio_data: Dict[str, Any], audio_duration: float) -> Dict[str, Any]:
        ...
        # Enhanced VAD parameters for accurate speech detection
        vad_parameters = {
            "threshold": self.config.get("faster_whisper_vad_threshold", 0.15),  # More conservative
            "min_silence_duration_ms": self.config.get("faster_whisper_min_silence_duration_ms", 500),  # Longer silence
            "max_speech_duration_s": self.config.get("faster_whisper_max_speech_duration_s", 30),  # Shorter segments
            "min_speech_duration_ms": self.config.get("faster_whisper_min_speech_duration_ms", 100)  # Minimum speech
        }
        ...
        try:
            segments_generator, info = self.model.transcribe(
                ...
                no_speech_threshold=0.4,  # Higher threshold for more conservative detection
                log_prob_threshold=-2.5,  # More conservative
            ...
            # Process segments with enhanced validation
            chunks = []
            last_end_time = 0.0
            segment_count = 0
            filtered_count = 0
            ...
            for segment in segments_generator:
                ...
                # Enhanced word-level filtering with reprocessing fallback
                if hasattr(segment, 'words') and segment.words:
                    ...
                    # Calculate average confidence of all words (for reprocessing decision)
                    all_words = [w for w in segment.words if w.word.strip()]
                    avg_all_confidence = np.mean([w.probability for w in all_words]) if all_words else 0.0

                    if not high_conf_words:
                        # No high confidence words - try reprocessing if enabled
                        if self.config.get("reprocess_low_confidence", True) and avg_all_confidence > 0.1:
                            print(
                                f"   🔄 Attempting reprocessing: '{text[:30]}...' (avg conf: {avg_all_confidence:.2f})")

                            reprocessed_result = self._reprocess_low_confidence_segment(
                                audio_data, float(segment.start), float(segment.end),
                                text, avg_all_confidence
                            )

                            if reprocessed_result["reprocessed"] and reprocessed_result[
                                "confidence"] > avg_all_confidence:
                                # Use reprocessed result
                                text = reprocessed_result["text"]
                                print(
                                    f"   ✅ Reprocessed successfully: conf {avg_all_confidence:.2f} → {reprocessed_result['confidence']:.2f}")

                                # Use original timing but mark as reprocessed
                                start = float(segment.start)
                                end = float(segment.end)
                            else:
                                # Reprocessing didn't help - use fallback or skip
                                if self.config.get("enable_fallback_transcription", True):
                                    print(
                                        f"   ⚠️  Using fallback for low-conf segment: '{text[:30]}...' (conf: {avg_all_confidence:.2f})")
                                    # Keep original but mark as low confidence
                                    start = float(segment.start)
                                    end = float(segment.end)
                                else:
                                    print(
                                        f"   ❌ Filtered low-confidence segment: '{text[:30]}...' (no high-conf words)")
                                    filtered_count += 1
                                    continue
                        else:
                            # Reprocessing disabled or confidence too low - skip
                            print(
                                f"   ❌ Filtered low-confidence segment: '{text[:30]}...' (no high-conf words, conf: {avg_all_confidence:.2f})")
                            filtered_count += 1
                            continue
                    else:
                        # Has high confidence words - calculate segment confidence
                        avg_confidence = np.mean([w.probability for w in high_conf_words])

                        if avg_confidence < segment_confidence_threshold:
                            # Low segment confidence - try reprocessing
                            if self.config.get("reprocess_low_confidence", True):
                                print(
                                    f"   🔄 Reprocessing low-conf segment: '{text[:30]}...' (conf: {avg_confidence:.2f})")
                                ...
                                if reprocessed_result["reprocessed"] and reprocessed_result[
                                    ...
                                    text = reprocessed_result["text"]
                                    print(
                                        f"   ✅ Reprocessed improvement: {avg_confidence:.2f} → {reprocessed_result['confidence']:.2f}")
                                else:
                                    # Keep original if no significant improvement
                                    print(f"   ⚠️  Keeping original: '{text[:30]}...' (conf: {avg_confidence:.2f})")
                            else:
                                print(
                            ...
                            print(f"   ✅ High-conf segment: conf: {avg_confidence:.2f}, '{text[:30]}...'")

                        # Use word-level timestamps for accuracy
                        word_start = float(high_conf_words[0].start)
                        word_end = float(high_conf_words[-1].end)

                        # Conservative timestamp adjustment
                        start = max(word_start - 0.1, float(segment.start))
                        end = min(word_end + 0.1, float(segment.end))
                else:
                    # No word timestamps - use segment timing with validation
                    start = float(segment.start)
                    end = float(segment.end)
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("\nEnhanced for ACCURATE TIMESTAMPS:")
        print("  🎯 Advanced Voice Activity Detection (VAD)")
        print("  🔍 Spectral analysis for speech vs silence")
        print("  ⏱️  Precise timestamp validation")
        print("  🚫 Background noise filtering")
        print("  🔗 Smart segment merging")
        print("  ✂️  Automatic silence removal")
        print("\nRequirements:")
        print("  pip install faster-whisper librosa srt scipy")
        print("\nKey Improvements:")
        print("  • Eliminates subtitles during silence")
        print("  • Validates speech content before creating subtitles")
        print("  • Conservative timing to match actual speech")
        sys.exit(1)
